...
function HomePage() {
  ...
  const [shadowHeight, setShadowHeight] = useState(100);
  const [shadowWidth, setShadowWidth] = useState(100);
  useEffect(() => {
    let pic = document.getElementById('pfp');
    console.log(document.getElementById('pfp').naturalWidth);
    setShadowHeight(pic.height);
    setShadowWidth(pic.width);
  }, [document.getElementById('pfp')]);

  return (
    <>
      <div id="section-1" className="p-2 text-pastelBlue-700 max-w-screen h-full m-auto">
        <div className="text-center md:text-left h-full">
          <div className="flex flex-col justify-around h-full">
            ...
            <div className="flex flex-row justify-center">
              <button className="bg-pastelBlue-500 hover:bg-pastelBlue-700 text-white font-bold py-2 px-4 rounded" style={{ height: 50, marginTop: 0 }}>
                LEARN MORE ABOUT ME
                </button>
            </div>
          </div>
